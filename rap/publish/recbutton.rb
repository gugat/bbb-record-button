...
if (playback == "recbutton")
	...
	audio = "#{process_dir}/audio.ogg"		
	webcam = "#{process_dir}/webcam.webm"		
	deskshare = "#{process_dir}/deskshare.webm"		
	package_dir = "#{target_dir}/#{meeting_id}"
	FileUtils.mkdir_p package_dir

	audio_dir = "#{package_dir}/audio"
	FileUtils.mkdir_p audio_dir
	FileUtils.cp("#{process_dir}/audio.ogg", audio_dir) if File.exists?(audio)

	video_dir = "#{package_dir}/video"
        FileUtils.mkdir_p video_dir
	FileUtils.cp(webcam, video_dir) if File.exists?(webcam)
	FileUtils.cp(deskshare, video_dir) if File.exists?(deskshare)
